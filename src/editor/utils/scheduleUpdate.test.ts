...
suite("scheduleUpdate", () => {
	...
	suiteSetup(async () => {
		previousMs = vscode.workspace
			.getConfiguration("sail")
			.get<number>("updateDebounceMs");
		await vscode.workspace
			.getConfiguration("sail")
			.update("updateDebounceMs", 0, vscode.ConfigurationTarget.Global);
	});
	...
	test("resets the timer when called again before firing", async () => {
		await vscode.workspace
			.getConfiguration("sail")
			.update("updateDebounceMs", 40, vscode.ConfigurationTarget.Global);
		try {
			const debounceTimer: {
				current: ReturnType<typeof setTimeout> | undefined;
			} = { current: undefined };
