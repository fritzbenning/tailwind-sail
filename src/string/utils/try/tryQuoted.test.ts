...
suite("tryDoubleQuote / trySingleQuote", () => {
	...
	test("trySingleQuote extracts when cursor inside; advances past literal when outside", () => {
		const text = "x = 'cd';";
		const open = text.indexOf("'");
		const inside = trySingleQuote(text, open, open + 2);
		assert.ok(inside.extracted);
		assert.strictEqual(inside.extracted!.rawContent, "cd");
		assert.strictEqual(inside.end, text.indexOf(";"));

		const before = trySingleQuote(text, open, 0);
		assert.strictEqual(before.extracted, undefined);
		assert.strictEqual(before.end, text.indexOf(";"));
	});

	test("trySingleQuote honors escapes and does not end on escaped quote", () => {
		const text = "x = 'a\\'b';";
		const open = text.indexOf("'");
	...
	test("trySingleQuote unterminated newline stops scan without extracted", () => {
		const text = "x = 'ab\n";
		const open = text.indexOf("'");
		const r = trySingleQuote(text, open, open + 3);
		assert.strictEqual(r.extracted, undefined);
