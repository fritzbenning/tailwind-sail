...
suite('getBreakpointSortTuple', () => {
	...
	test('treats arbitrary min-[…] / max-[…] as tier 1', () => {
		assert.deepStrictEqual(getBreakpointSortTuple('min-[500px]'), [1, 0, 0, 'min-[500px]']);
		assert.deepStrictEqual(getBreakpointSortTuple('max-[100vh]'), [1, 0, 0, 'max-[100vh]']);
	});

	test('falls back to catch-all tier for unknown keys', () => {
		assert.deepStrictEqual(getBreakpointSortTuple('custom-bp'), [2, 0, 0, 'custom-bp']);
	});
