...
suite("getActiveVariantClasses", () => {
	...
	test("theme light chip is ignored (light + hover → hover:)", () => {
		...
		eff.state = "hover";
		assert.strictEqual(
			getActiveVariantClasses(new Set(["theme", "state"]), eff),
			"hover:",
		);
	});
	...
	test("skips synthetic chips that are not literal class prefixes", () => {
		...
		assert.strictEqual(
			getActiveVariantClasses(
				new Set(["state", "breakpoints", "container", "theme"]),
				eff,
			),
			"",
		);
	...
	test("includes breakpoint and arbitrary-style keys with colons", () => {
		...
		eff.breakpoints = "md";
		eff.other = "[&_svg]";
		const prefix = getActiveVariantClasses(
			new Set(["breakpoints", "other"]),
			eff,
		);
