...
import type { SailWebviewPanelModel, SailWebviewViewModel } from '@sail/protocol';
...
function mergeFilter(prev: ClientFilterState, patch: Partial<ClientFilterState>): ClientFilterState {
	return {
...
export function App() {
	const [model, setModel] = createSignal<SailWebviewViewModel>({ kind: 'needString' });
	const [filter, setFilter] = createSignal<ClientFilterState>(defaultClientFilterState());
	...
	onMount(() => {
		...
		const onFocusIn = (e: Event) => {
			const t = e.target;
			if (t instanceof HTMLInputElement && t.classList.contains('class-token-input')) {
				deferRender = true;
			}
		};
		const onFocusOut = (e: Event) => {
			...
			setTimeout(() => {
				...
				if (ae instanceof HTMLInputElement && ae.classList.contains('class-token-input')) {
					return;
				}
				deferRender = false;
				if (pendingModel) {
