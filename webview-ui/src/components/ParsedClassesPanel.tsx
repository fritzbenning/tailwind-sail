...
export function ParsedClassesPanel(props: {
	...
	const onSemanticChip = (id: string) => {
		...
		const nextSem =
			st.semantic.t === 'semantic' && st.semantic.v === id ? { t: 'all' as const } : { t: 'semantic' as const, v: id };
	...
	const showNoResult = () =>
		props.panel.classes.length > 0 && !anyClassVisible(props.panel, props.filter);
	...
	const addClassStripThemeLight = createMemo(
		() => effectiveVariantState(props.panel, props.filter.variant).theme === 'light',
	);
	...
	return (
		<div class="parsed-classes-panel">
			...
			<SemanticFilterBar panel={props.panel} semantic={props.filter.semantic} onSemanticChip={onSemanticChip} />
			<VariantFilterRows panel={props.panel} variant={props.filter.variant} onVariantChip={onVariantChip} />
			<Show when={props.panel.showVariantPrefixToggle}>
				<VariantPrefixToggle
					checked={props.filter.hideMatchingVariantPrefixes}
					onChange={(checked) => props.onPatchFilter({ hideMatchingVariantPrefixes: checked })}
				/>
			</Show>
			...
			<div class="sail-class-list-scroll">
				<ul class="class-token-list">
					<Show when={props.panel.classes.length === 0}>
						<li class="class-row muted" data-sail-no-token="true" data-sail-semantic="others">
							(no classes)
						</li>
					</Show>
